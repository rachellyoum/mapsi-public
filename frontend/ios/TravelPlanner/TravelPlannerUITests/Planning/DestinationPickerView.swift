...
struct DestinationPickerView: View {
    ...
    private func isRelevantPlaceResult(_ place: PlaceSearchResult, query: String, selectedCity: String) -> Bool {
        ...
        let normalizedAddress = (place.address ?? "").lowercased()
        let normalizedCity = selectedCity.lowercased()
        ...
        let looksLikeCityOnlyResult = normalizedName.contains(normalizedCity)
            && normalizedAddress.contains(normalizedCity)
            && !normalizedName.contains(normalizedQuery)

        return !looksLikeCityOnlyResult
    }
